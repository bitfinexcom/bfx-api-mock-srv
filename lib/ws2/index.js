...
class MockWSv2Server extends MockServer {
   ...
   *   connect
   * @param {Function} [args.authMiddleware=] - handle auth response
   * @param {boolean} [args.listen=true] - if true, listen() is called
   *   automatically
   */
  constructor (args = {}) {
    const {
      apiPort = 9997, cmdPort = 9996, syncOnConnect = true, authMiddleware, listen = true
    } = args
  ...
  _onClientMessage (ws, msgJSON) {
    ...
    if (msg.event === 'auth') {
      this._handleAuthMessage(ws, msg)
   ...
   *
   * @param {WS.WebSocket} ws - ws client
   * @param {object} msg - request's message
   */
  _handleAuthMessage (ws, msg) {
    const canAuth = !this._authMiddleware || this._authMiddleware(msg)
