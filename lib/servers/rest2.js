...
class REST2Server extends Server {
  constructor (args = { apiPort: 9999, cmdPort: 9998, listen: true }) {
    ...
    Object.keys(PUBLIC_METHODS).forEach((route) => {
      this._generateRoute('get', route, PUBLIC_METHODS[route])
    })

    Object.keys(AUTH_METHODS).forEach((route) => {
      this._generateRoute('post', route, AUTH_METHODS[route])
    })
