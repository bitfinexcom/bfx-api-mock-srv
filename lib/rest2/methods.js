...
const RESTV2_MOCK_METHODS = {
  ...
  '/v2/conf/pub([:])map([:])currency([:])label\*': 'map_ccy_label', // eslint-disable-line
  ...
  '/v2/conf/pub([:])list([:])currency([:])futures\*': 'futures_currencies', // eslint-disable-line
  '/v2/conf/pub([:])list([:])currency([:])securities\*': 'securities_currencies', // eslint-disable-line
  '/v2/conf/pub([:])list([:])currency([:])margin\*': 'margin_currencies', // eslint-disable-line
  ...
  '/v2/platform/status': 'platform_status',
  ...
  '/v2/calc/fx': 'calc_fx',
  '/v2/auth/r/trades/calc': 'weighted_averages.{symbol}.{start}.{end}.{limit}'
}
